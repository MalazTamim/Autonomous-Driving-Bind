...
class NuScenes:
    ...
    def render_pointcloud_in_image_black(self, sample_token: str, dot_size: int = 5, pointsensor_channel: str = 'LIDAR_TOP',
                                   camera_channel: str = 'CAM_FRONT', out_path: str = None,
                                   render_intensity: bool = False,
                                   show_lidarseg: bool = False,
                                   filter_lidarseg_labels: List = None,
                                   ...
                                   lidarseg_preds_bin_path: str = None,
                                   show_panoptic: bool = False) -> None:
        self.explorer.render_pointcloud_in_image_black(sample_token, dot_size, pointsensor_channel=pointsensor_channel,
                                                 camera_channel=camera_channel, out_path=out_path,
                                                 render_intensity=render_intensity,
                                                 show_lidarseg=show_lidarseg,
                                                 filter_lidarseg_labels=filter_lidarseg_labels,
    ...
    def render_pointcloud_in_image_black_radar(self,
                                   sample_token: str,
                                   dot_size: int = 5,
                                   ...
                                   lidarseg_preds_bin_path: str = None,
                                   show_panoptic: bool = False) -> None:
        self.explorer.render_pointcloud_in_image_black_radar(sample_token, dot_size, pointsensor_channel=pointsensor_channel,
                                                 camera_channel=camera_channel, out_path=out_path,
...
class NuScenesExplorer:
    ...
    def render_pointcloud_in_image_black(self,
                                   sample_token: str,
                                   dot_size: int = 5,
                                   pointsensor_channel: str = 'LIDAR_TOP',
                                   camera_channel: str = 'CAM_FRONT',
    ...
    def render_pointcloud_in_image_black_radar(self,
        ...
        radar_points, radar_coloring, _ = self.map_pointcloud_to_image(
        ...
        points, lidar_coloring, lidar_im = self.map_pointcloud_to_image(pointsensor_token, camera_token,
                                                            render_intensity=render_intensity,
                                                            show_lidarseg=show_lidarseg,
                                                            filter_lidarseg_labels=filter_lidarseg_labels,
                                                            lidarseg_preds_bin_path=lidarseg_preds_bin_path,
        ...
        # Combine lidar and radar depth images
        combined_depth_image = np.maximum(points, radar_points)
        # plt.imshow(np.array(lidar_depth_path))
        # img = Image.open(lidar_depth_path)
        # black_im = mpimg.imread(lidar_depth_path)
        # black_im = Image.open(lidar_depth_path)
        # print("Loaded image size:", black_im.size)
        black_im = Image.new('RGB', im.size, (0, 0, 0))
        # print(black_im.size)
        # Init axes.
        if ax is None:
        ...
        ax.imshow(black_im)
        ax.scatter(combined_depth_image[0, :], combined_depth_image[1, :], c=coloring, s=dot_size)
        ax.axis('off')
        
        # Create a drawing object
        # draw = ImageDraw.Draw(black_im)
        
        # Scatter plot on top of the loaded image
        # draw.scatter(points[0, :], points[1, :], c=coloring, s=dot_size)
        # Scatter plot on top of the loaded image
        # for i in range(points.shape[1]):
        #     x, y = int(points[0, i]), int(points[1, i])
        #     color = coloring_int[i]
