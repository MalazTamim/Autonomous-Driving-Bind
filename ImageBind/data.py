...
def load_and_transform_depth_data2(image_paths, device, to_tensor=True):
    if image_paths is None:
        return None
    ...
    for image_path in image_paths:     
        with open(image_path, "rb") as fopen:
            image = np.array(Image.open(fopen).convert("L"))
...
def load_and_transform_depth_data_any(depth_paths, device):
...
def load_and_transform_depth_data(depth_paths, device, to_tensor=True):
    ...
    thermal_ouputs = []
    for thermal_path in depth_paths:
        thermal = cv2.imread(thermal_path, cv2.IMREAD_UNCHANGED).astype('float32')
        if to_tensor:
    ...
    #                 ),
        ...
        if to_tensor:
            ...
            thermal = data_transform(thermal)
            thermal_ouputs.append(thermal)
    return thermal_ouputs if not to_tensor else torch.stack(thermal_ouputs, dim=0)
