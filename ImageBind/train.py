...
# Set  WandB API key here
wandb_api_key = "4f084865b8fb0fc568b09b197794d45d4433f015"  # Replace with actual WandB API key
...
#Applies transformation to a certain dataset
...
class ImageBindTrain(L.LightningModule):
    def __init__(self, lr=5e-6, weight_decay=1e-4, max_epochs=500, batch_size=32, num_workers=4, seed=42, 
                 self_contrast=False, temperature=0.07,  momentum_betas=(0.9, 0.95), 
                 lora=False, lora_rank=4, lora_checkpoint_dir="./.checkpoints/lora/imagebind-mini",
...
def parse_args():
    ...
    parser.add_argument("--loggers", type=str,default="wandb", nargs="+", choices=["tensorboard", "wandb", "comet", "mlflow"],#default="tensorboard"
    ...
    parser.add_argument("--lora_checkpoint_dir", type=str, default="./.checkpoints/lora/imagebind-mini",
    ...
    parser.add_argument("--captions", type=str, nargs="+", #default=["dreambooth"], choices=["dreambooth"],
                        help="Captions version")
...
if __name__ == "__main__":
    ...
    for logger in args.loggers if args.loggers is not None else []:
        if logger == "wandb":
            wandb.init(project="imagebind-mini", config=args)
    ...
    #         root_dir=os.path.join(args.datasets_dir, "dreambooth"), split="train",
    ...
    #         root_dir=os.path.join(args.datasets_dir, "dreambooth"), split="test",
    #         transform=ContrastiveTransformations(contrast_transforms,
    #                                              n_views=2 if args.self_contrast else 1)))
    if "nuscenes" in args.datasets and "updated-captions" in args.captions :
        from datasets.nuscenes import NuscenesDatasetFinalCaptions
        train_datasets.append(NuscenesDatasetFinalCaptions(
            root_dir=os.path.join(args.datasets_dir, "nuscenes"), split="train",
            transform=ContrastiveTransformations(contrast_transforms,
                                                 n_views=2 if args.self_contrast else 1)))
        test_datasets.append(NuscenesDatasetFinalCaptions(
            root_dir=os.path.join(args.datasets_dir, "nuscenes"), split="test",
            transform=ContrastiveTransformations(contrast_transforms,
                                                 n_views=2 if args.self_contrast else 1)))
    
    elif "nuscenes" in args.datasets:
        from datasets.nuscenes import NuscenesDataset
        train_datasets.append(NuscenesDataset(
            root_dir=os.path.join(args.datasets_dir, "nuscenes"), split="train",
            transform=ContrastiveTransformations(contrast_transforms,
                                                 n_views=2 if args.self_contrast else 1)))
        test_datasets.append(NuscenesDataset(
            root_dir=os.path.join(args.datasets_dir, "nuscenes"), split="test",
            transform=ContrastiveTransformations(contrast_transforms,
                                                 n_views=2 if args.self_contrast else 1)))
        ...
        checkpointing = {"enable_checkpointing": args.full_model_checkpointing,
                         "callbacks": [
                           ...
                           save_weights_only=True,  # Save only the model weights, not the entire model
                            ...
                            EarlyStopping(
                                monitor="val_loss",
                                patience=8,  # Set the patience value as needed
                                mode="min"
                                )
                        ]}
    else:
    ...
    dirpath=args.full_model_checkpoint_dir,
    filename='imagebind-epoch=11-val_loss=0.88.ckpt',
    save_last=True,
    mode='min',
)

    trainer = Trainer(accelerator="gpu" if "cuda" in device_name else "cpu",
                      devices=1 , deterministic=True,#if ":" not in device_name else [int(device_name.split(":")[1])]
